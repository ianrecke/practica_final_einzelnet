...
def result():
    if request.method == 'POST':
        ...
        return render_template("result.html", fecha1=fecha,result=round(result[0][0],3)*100,result2 = round(result2[0][0],3)*100,fecha2 = fecha2,diferencia_pais1 = round(diferencia_pais1[0][0],3),diferencia_pais2 = round(diferencia_pais2[0][0],3),pais1 = to_predict_list[2],pais2 = to_predict_list[3],casos_pais1 = casos_pais1,casos_pais2 = casos_pais2,sexo = int(datos_uci[0]),intubacion = convierteString(datos_uci[1]),neumonia = convierteString(datos_uci[2]),edad = datos_uci[3],embarazo = convierteString(datos_uci[4]),diabetes = convierteString(datos_uci[5]),asma = convierteString(datos_uci[6]),inmunosupresores = convierteString(datos_uci[7]),hipertension = convierteString(datos_uci[8]),otra_enf = convierteString(datos_uci[9]),cardiovascular = convierteString(datos_uci[10]),bmi = datos_uci[11],renal = convierteString(datos_uci[12]),fumador = convierteString(datos_uci[13]),contacto = convierteString(datos_uci[14]),estatura = to_predict_list[15],peso = to_predict_list[16],resultado_nv = convierteString(naive),array = to_predict_list)
